...
if [ -z "$(ls "$PETSC_SRC/$PETSC_ARCH/lib/libpetsc.so."* 2>/dev/null)" ]; then
    echo ">>> Building PETSc"
    cd "$PETSC_SRC"
...
if [ -z "$(ls "$SLEPC_SRC/$PETSC_ARCH/lib/libslepc.so."* 2>/dev/null)" ]; then
    echo ">>> Building SLEPc"
    cd "$SLEPC_SRC"
...
if ! python3 -c "import petsc4py" &>/dev/null; then
    ...
    pip cache purge
    pip install --no-binary=petsc4py --no-binary=petsc --no-deps petsc4py
else
...
if ! python3 -c "import slepc4py" &>/dev/null; then
    ...
    pip cache purge
    pip install --no-binary=slepc4py --no-binary=slepc --no-deps slepc4py
else
